...
const Accessories = () => {
  ...
  // Stable list of 30 products
  const products = useMemo(() => Array.from({ length: 30 }, (_, i) => ({
    id: i + 200, // Unique ID offset from men's and women's apparel
    name: `Style Accent ${i + 1}`,
    price: `R ${(150 + (i * 12.5)).toFixed(2)}`,
    image: accImg,
  })), []);
      ...
      <div className="mx-auto max-w-[1400px] px-6 py-20 md:px-10">
        <div className="flex items-center gap-4 mb-10">
          ...
          <h1 className="font-display text-5xl font-bold">
            Accessories <em className="italic text-muted-foreground">& Details</em>
          </h1>
        </div>
            ...
            <Card key={product.id} className="overflow-hidden border-none shadow-none group bg-secondary/20">
              ...
              <CardFooter className="p-4 pt-0 flex gap-2">
                ...
                <Button 
                  ...
                  className={favorites.find(f => f.id === product.id) ? "text-red-500 border-red-500" : ""}
                >
